        ...
        foreach ($mv as $row) { ?>
            ...
            <section class="movie-grid">
                ...
                if (isset($_POST['addList'])) {
                    ...
                    $ty = $row['type'];
                    $time = $row['time'];
                    $sql = 'INSERT INTO playList( title , image , type , time) VALUE( ? , ? , ? , ? )';
                    $stmt = $con->prepare($sql);
                    $stmt->execute(array($n, $img, $ty, $time));
                }
                foreach ($tab  as $row) {
                ?>

                    <form action="?action=playList" method="post">
                        <div class="card bg-dark rounded row" style="width: 12rem;">
                            ...
                            <div class="card-body">
                                ...
                                <button name="addList" class="btn btn-primary"><i class='bx bx-plus'></i>Addlist</button>
